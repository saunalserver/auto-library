...
def download_album(artist, album, checker):
    search_query = f'{artist} {album}'
    log(f'Downloading: {search_query}')
    cmd = [TIDDL_BINARY, 'search', search_query, 'download']
    try:
        ...
        if 'no search results' in combined_output.lower():
        ...
        error_msg = combined_output[:200] if combined_output else f"Exit code {result.returncode}"
        log(f'Failed: {artist} - {album}: {error_msg}', 'ERROR')
